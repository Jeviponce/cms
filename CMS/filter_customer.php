...
if (isset($_POST['submit'])) {
    if ($year && !$month) {
        $filter = "SELECT * FROM user_tbl WHERE YEAR(dte) = '$year' AND  userlvl='0'";
        renderTableBody($conn, $filter);
    }
    else if($month && !$year) {
        $filter = "SELECT * FROM user_tbl WHERE MONTH(dte) = '$month' AND  userlvl='0'";
        renderTableBody($conn, $filter);
    }
    else if($month && $year) {
        $filter = "SELECT * FROM user_tbl WHERE MONTH(dte) = '$month' AND YEAR(dte) = '$year' AND userlvl='0'";
        renderTableBody($conn, $filter);
    }else{
...
<?php 
function renderTableBody($conn, $query = "SELECT * FROM user_tbl WHERE userlvl='0'") {
